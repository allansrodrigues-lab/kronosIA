"""
Proposta comercial Kronos Intelligence — Serviço de Implantação de IA Operacional (Claude + MCP).
Para usar: alterar apenas o bloco DADOS DO CLIENTE abaixo.
Rodar: uv run --with reportlab python gerar_proposta_implementacao.py
"""
...
CLIENTE_PROPRIETARIO = "Nome do responsável"
CLIENTE_ENDERECO     = "Endereço da empresa — Cidade/UF"
IMPLANTACAO_VALOR    = "R$ 1.990,00"
MENSALIDADE_VALOR    = "R$ 690,00/mês"
IMPLANTACAO_DESC     = "Diagnóstico + configuração do processo-piloto,\nteste com dado real e treinamento da equipe (pagamento único)"
MENSALIDADE_DESC     = "Suporte, ajuste contínuo e monitoramento do processo em produção"

PROBLEMA = (
    "Toda semana a sua equipe repete a mesma tarefa manual — copiar dado de um lugar pro outro, "
    "montar o mesmo relatório, responder o mesmo tipo de e-mail. Essas horas não geram diferencial "
    "nenhum pra empresa, só consomem tempo que a pessoa poderia usar em algo que exige julgamento real."
)

SOLUCAO = (
    "A Kronos entra na empresa, identifica junto com você qual processo repetitivo vale automatizar "
    "primeiro, e configura o Claude conectado às ferramentas que a empresa já usa — Gmail, planilha, "
    "Drive, agenda. Não é um robô genérico: é IA configurada pro seu processo real, testada com seu "
    "dado real, e a pessoa que já faz essa tarefa aprende a operar em vez de continuar fazendo na mão."
)

COMO_FUNCIONA = [
    "Chamada de diagnóstico: mapeamos junto qual processo tem maior retorno e menor risco pra começar",
    "Configuramos o Claude conectado às ferramentas reais da empresa (MCP), com o mínimo de acesso necessário",
    "Testamos com dado real da empresa — não com exemplo genérico",
    "Treinamos a pessoa que vai operar no dia a dia",
    "Acompanhamos as primeiras semanas de uso real e ajustamos exceção que aparecer",
]

DIFERENCIAIS = [
    "Não vende IA genérica — resolve um processo específico, escolhido junto com você",
    "Conecta nas ferramentas que a empresa já usa, sem trocar de sistema",
    "Código faz a parte de cálculo, IA faz a parte de linguagem — número nunca sai de invenção",
    "Toda saída que sai da empresa passa por revisão humana no primeiro mês",
    "Treinamento de quem usa, não só entrega técnica pra TI",
    "Expansão gradual — um processo por vez, com ROI comprovado antes do próximo",
]

INCLUSO_IMPL = [
    "Chamada de diagnóstico e escolha do processo-piloto",
    "Configuração do Claude + MCP nas ferramentas da empresa",
    "Teste com dado real da empresa",
    "Treinamento da pessoa responsável",
    "Documentação enxuta de uso",
    "Acompanhamento do go-live",
]

INCLUSO_MENS = [
    "Suporte contínuo de ajuste",
    "Correção de exceção que aparecer no uso real",
    "Monitoramento do processo em produção",
    "Prioridade de atendimento",
    "Espaço pra avaliar o próximo processo a automatizar",
    "Relatório mensal de horas economizadas",
]

ROI_TEXTO = (
    "Um analista dedicado só a essa tarefa custa entre R$ 2.000 e R$ 3.000 por mês trabalhando "
    "exclusivamente nisso. A implementação não substitui a pessoa — libera ela pra fazer o que exige "
    "julgamento humano de verdade, e o retorno aparece já nas primeiras semanas em horas devolvidas "
    "pra equipe."
)
# ══════════════════════════════════════════════════════════════════

OUTPUT = os.path.join(os.path.dirname(__file__), 'proposta_implementacao.pdf')
LOGO   = os.path.join(os.path.dirname(__file__), '..', '07_Recursos', 'identidade_visual',
                       'kronos_logo_oficial_transparente.png')
